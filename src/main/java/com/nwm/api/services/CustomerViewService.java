...
public class CustomerViewService extends DB {
	...
	public <T> DevicesByTypeEntity getDevicesBySite(T obj) {
		try {
			...
			List<DeviceEntity> irradianceDevices = devices.stream().filter(item -> ((item.getId_device_type() == 4) && item.getReverse_poa() == 0) || (item.getId_device_type() == 21)).collect(Collectors.toList());
